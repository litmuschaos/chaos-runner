...
func main() {
	...
	if err = clients.GenerateClientSets(config); err != nil {
		klog.V(1).Infof("Unable to generate clientSet, due to error: %v", err)
	}
	...
	for i := range engineDetails.Experiments {
		...
		klog.V(2).Infof("Setting Values from ChaosEngine")
		experiment.SetValueFromChaosEngine(engineDetails, i)
		klog.V(2).Infof("Values set are Experiment Name: %v, Experiment Namespace: %v, Experiment ServiceAccount: %v", experiment.Name, experiment.Namespace, experiment.SvcAccount)

		klog.V(2).Infof("Setting Value from ChaosExperiment")
		experiment.SetValueFromChaosExperiment(clients)
		klog.V(2).Infof("Values set are Experiment Image: %v, Experiment Args: %v, Experiment Labels: %v, Experiment JobName: %v", experiment.ExpImage, experiment.ExpArgs, experiment.ExpLabels, experiment.JobName)

		klog.V(2).Infof("Setting ENV Values for ChaosExperiment")
		experiment.SetENV(engineDetails, clients)
		klog.V(2).Infof("ENV Values set in ChaosExperiment are: %v", experiment.Env)

		experimentStatus := utils.ExperimentStatus{}

		klog.V(2).Infof("Creating Intial Experiment Status")
		experimentStatus.IntialExperimentStatus(experiment)

		klog.V(1).Infof("ChaosEngine Initial Patching will be done for all the Experiment including the one not found also.")
		experimentStatus.InitialPatchEngine(engineDetails, clients)

		klog.V(0).Infof("Perparing to run Chaos Experiment: %v", experiment.Name)
		klog.V(2).Infof("Printing Experiment Stucture: %v", experiment)
		// isFound will return the status of experiment in that namespace
		// 1 -> found, 0 -> not-found
		isFound, err := experiment.CheckExistence(clients)
		klog.V(0).Infof("Chaos Experiment Validated in Application NamespaceL %v", experiment.Namespace)
		klog.V(1).Infof("Experiment Found Status : %v", isFound)

		// If not found in AppNamespace skip the further steps
		if !isFound {
			klog.V(1).Infof("Will try to patch ChaosEngine, with Not Found Status for Experiment: %v", experiment.Name)
			engineDetails.ExperimentNotFoundPatchEngine(experiment, clients)
			klog.V(0).Infof("Unable to list Chaos Experiment: %v, in Namespace: %v, skipping execution, with error: %v", experiment.Name, experiment.Namespace, err)
			break
		}

		// Patch ConfigMaps to ChaosExperiment Job
		if err := experiment.PatchConfigMaps(clients); err != nil {
			klog.V(0).Infof("Unable to patch ConfigMaps, due to: %v", err)
			break
		}

		// Patch Secrets to ChaosExperiment Job
		if err = experiment.PatchSecrets(clients); err != nil {
			klog.V(0).Infof("Unable to patch Secrets, due to: %v", err)
			break
		}
